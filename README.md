...
取消锚点恢复原结果，以及备选对齐的旧请求零新增字段 / 同代价与严格更贵
次优路径按确定顺序选出 / 首处分歧索引 / 唯一解空备选提示 / 锚点同时
约束两条路径 / 非布尔开关单次报错），通过后退出码为 0：
...
覆盖与后端一致的客户端校验（含锚点校验镜像）、带源码位置的 JSON 解析器、
API 客户端（锚点可选序列化/回显、`compare_alternative` 仅在开启时发送、
备选索引的 bigint 归一化）、错误路径高亮、输入与已选标记保留、
锚点点选/取消交互、人工锚点与算法行区分、逐步复算，以及备选摘要、
分歧行展开、同价/严格更贵备选与唯一解提示。

### Playwright — 真实全栈联调（19 个用例）
...
锚点同时约束备选路径、开关缺省时请求与页面均无任何备选分析。也可设置
...
校验 `anchors`，最后才检查 `compare_alternative`，因此任何输入组合都只有
**首个**错误被报告。成功响应在开启比较时于原有字段后追加 `alternative`
（唯一解为 `null`），关闭时响应结构与以往完全一致。
...
├── backend/
│   ├── app/
│   │   ├── alignment.py      # DP 对齐（代价矩阵 + 平局裁决 + 回溯 + 锚点切分 + 2-best 次优路径）
│   │   ├── validation.py     # 结构/数量/类型/递增/锚点/开关校验，返回首个错误路径
│   │   └── main.py           # FastAPI、手工 JSON 解析、单次失败契约
│   ├── tests/                # pytest：算法（含锚点/次优受限对拍）、校验、真实 ASGI API
│   ├── Dockerfile            # python:3.12-slim
│   └── requirements*.txt
├── frontend/
│   ├── src/
│   │   ├── App.tsx           # 页面：输入、锚点点选、错误定位、逐步复算
│   │   ├── AnchorPanel.tsx   # 人工锚点标记/取消面板
│   │   ├── ResultTimeline.tsx# 逐行来源（人工锚点·算法生成）/动作/代价/复算
