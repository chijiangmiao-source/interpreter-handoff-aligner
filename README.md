...
以及锚点的无锚点结果不变 / 合法锚点固定入列且区间最优 / 交叉单次报错 /
取消锚点恢复原结果），通过后退出码为 0：
...
（配对 > 左留空 > 右留空）的专门用例，以及锚点的**受限最优对拍**
（400 组随机 + 区间独立递归）、锚点越界/复用/交叉的单次失败契约和
「去锚点恢复原结果」用例。

### Vitest — 前端逻辑与组件（57 个用例）
...
API 客户端（锚点可选序列化/回显）、错误路径高亮、输入与已选标记保留、
锚点点选/取消交互、人工锚点与算法行区分及逐步复算。

### Playwright — 真实全栈联调（14 个用例）
...
非法 JSON 的行列号定位、空数组零代价交接，以及锚点固定入列并与算法行
区分、强制非显然配对、交叉锚点单次报错且不显示新时间轴、取消锚点恢复
原结果、无锚点请求不含 anchors。也可设置
...
校验 `anchors`，因此任何输入组合都只有**首个**错误被报告。
...
├── backend/
│   ├── app/
│   │   ├── alignment.py      # DP 对齐（代价矩阵 + 平局裁决 + 回溯 + 锚点切分）
│   │   ├── validation.py     # 结构/数量/类型/递增/锚点校验，返回首个错误路径
│   │   └── main.py           # FastAPI、手工 JSON 解析、单次失败契约
│   ├── tests/                # pytest：算法（含锚点受限对拍）、校验、真实 ASGI API
